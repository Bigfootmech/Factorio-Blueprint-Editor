...
local function get_var(direction, bool_enhance)
    local basic_vector = directions[direction]
    if bool_enhance then 
...
Event_Registration.register_command_from_action_definition({
    ...
    [Util.linked_function_field_name] = Api.edit_or_reopen_blueprint})
...
Event_Registration.register_command_from_action_definition({
    ...
    [Util.linked_function_field_name] = Api.add_inner_blueprint})
...
local function generate_line_for_direction_combination(direction_name, bool_enhance)
    ...
    [Util.linked_function_field_name] = Api.move_inner_blueprint(get_var(direction_name, bool_enhance))} -- circular reference part 2 RIP (currently defunct)
    
    return direction_action_definition
end

for direction_name, _ in pairs(directions) do
    for _,bool_enhance in pairs {true, false} do
